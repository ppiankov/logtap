...
- `--grep` now defaults to instant speed (load all lines immediately) — use `--speed 1` to override
- `0` key in replay TUI jumps to instant speed
- `]` past 64x now reaches instant speed
...
- Service summary header in replay TUI — shows services with line counts above progress bar

## [1.9.7] - 2026-03-07

### Added

- TUI bookmark lines — `m` + letter to set, `'` + letter to jump back
...
- TUI help overlay — press `?` to see all keybindings without leaving the dashboard

## [1.8.0] - 2026-03-06

### Added

- TUI grep mode (`/=pattern`) — shows only lines matching the pattern, status bar shows `GREP: /pattern [N lines]`
- Line count shown in status bar for HIDE and GREP modes
